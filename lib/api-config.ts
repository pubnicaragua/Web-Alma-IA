...
export const fetchWithAuth = async (endpoint: string, options: RequestInit = {}): Promise<Response> => {  
  ...
  const shouldAddSchoolId = !endpoint.endsWith('/colegios') &&   
  ...
  try {  
    ...
    if (!response.ok) {  
      ...
      throw new ApiError(`Error en la solicitud: ${errorDetails}`, response.status)  
    }  
...
export const fetchApi = async (endpoint: string, options: RequestInit = {}): Promise<Response> => {  
  ...
  console.log(`api-config:Enviando solicitud sin auth ${method} a ${API_BASE_URL}${normalizedEndpoint}`)  
