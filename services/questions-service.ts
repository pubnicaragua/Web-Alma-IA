...
function mapApiQuestionsToQuestions(apiQuestions: ApiQuestion[]): Question[] {
  return apiQuestions.map((apiQuestion) => {
    ...
    if (
      diagLower.includes("urgente") ||
      diagLower.includes("crítico") ||
      diagLower.includes("grave")
    ) {
      priority = 1; // Alta prioridad
    } else if (
      diagLower.includes("leve") ||
      diagLower.includes("seguimiento") ||
      diagLower.includes("rutina")
    ) {
...
const sampleQuestions: Question[] = [
  ...
  },
  {
    ...
    time: "PM",
    questionText:
      "Del 1 al 5, ¿qué tan nervioso te sientes antes de un examen?",
    priority: 2,
  },
  {
    id: "3",
  ...
  },
  {
    ...
    time: "PM",
    questionText:
      "¿Cómo te has sentido emocionalmente durante la última semana?",
    priority: 1,
  },
  {
    id: "5",
