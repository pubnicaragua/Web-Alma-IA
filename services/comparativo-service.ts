...
export async function getComparativaEmotionsCourses(): Promise<any> {
  try {
    console.log("Obteniendo Comparativas de emociones...")
    ...
    if (!response.ok) {
      ...
      console.error(`Error al obtener comparativas: ${response.status} - ${errorText}`)
      throw new Error(`Error al obtener comparativas: ${response.status} - ${errorText}`)
    }
